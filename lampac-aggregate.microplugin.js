(function(){
  ...
  function parseToVideos(htmlStr){
    // если вдруг пришёл JSON с rch — отдаём пусто (агрегатор пропустит такой источник)
    try{
      var maybe = JSON.parse(htmlStr);
      if (maybe && typeof maybe === 'object' && maybe.rch) return [];
    }catch(_){}
  ...
  function qualityOf(v){
    ...
    if (v && v.quality && typeof v.quality === 'object'){
      ...
      if (keys.length){
        q = Math.max.apply(null, keys.map(function(k){
          var n = parseInt(k);
          if (!isNaN(n)) return n;
          return /4k/i.test(k) ? 2160 : 0;
        }));
      }
    } else if (v && v.text){
  ...
  function ComponentAgg(object){
    ...
    var keys    = []; // порядок активных источников
    var lastFocus;
    ...
    this.reset = function(){
      ...
      scroll.clear();
      scroll.reset();
      var loaderTpl = Lampa.Template.get('lampac_content_loading');
      if (loaderTpl && loaderTpl.length) scroll.body().append(loaderTpl);
      else scroll.body().append('<div class="online-empty"><div class="broadcast__scan"><div></div></div></div>');
    };
    ...
    this.initialize = function(){
      ...
      this.externalids()
        .then(function(){ return _this.fetchSources(); })
        .then(function(){ _this.aggregateAll(); })
        ...
        network.silent(account(url), function(json){
          ...
          function startList(list){
            ...
            (list||[]).forEach(function(j){
              var base = (j.name||'').split(' ')[0] || '';
              var name = ((j.balanser||base)+'').toLowerCase();
          ...
          if (json && json.life){
            ...
            (function pull(){
              ...
              network.silent(account(lifeUrl), function(life){
                ...
                tries++;
                if (tries>10) reject({msg:'lifeevents timeout'});
                else setTimeout(pull,1000);
              });
            })();
          } else {
    ...
    this.aggregateAll = function(){
      ...
      status.onComplite = function(){
        ...
        // дедуп по сезон/серия/название/озвучка
        all = uniqBy(all, function(v){
          return [
            (v.season||0),
            (v.episode||0),
            String(v.title||v.text||'').toLowerCase(),
            String(v.voice_name||'').toLowerCase()
          ].join('|');
        });

        // сортировка: качество ↓, затем сезон/серия ↑
      ...
      function pump(){
        while(inflight<CONC && queue.length){
          ...
          network.native(url, function(text){
            var vids = [];
            try{
              vids = parseToVideos(text) || [];
              vids.forEach(function(v){
                v.balanser_key  = k;
                v.balanser_name = sources[k].name;
                v._source_page  = url;
              });
              results[k] = vids;
            }catch(e){
              ...
              inflight--; status.append(k, results[k]); pump();
            }
          }, function(){
            inflight--; results[k]=[]; status.append(k, []); pump();
          }, false, {dataType:'text'});
        }
      }
      pump();
    ...
    this.draw = function(items){
      ...
      items.forEach(function(element){
        ...
        // UI из ONLINE, если нет — простой фолбэк
        ...
        tpl.on('hover:enter', function(){
          // Откроем оригинальный ONLINE на нужном источнике
          Lampa.Activity.push({
    ...
    this.empty = function(msg){
      var html = Lampa.Template.get('lampac_does_not_answer',{}) || $('<div class="online-empty"><div class="online-empty__title"></div><div class="online-empty__time"></div></div>');
    ...
    head.find('.filter--sort span').text('Все источники');
  ...
  function addButton(ctx){
    if (!ctx || !ctx.render || !ctx.movie) return;

    // если уже добавляли — выходим
    if (ctx.render.parent().find('.lampac--agg-button').length) return;
    ...
    btn.on('hover:enter', function(){
      ...
      Lampa.Activity.push({
        url:'',
        title:'Онлайн (агрегатор)',
        component:'lampac_agg',
        movie: ctx.movie,
        page:1,
        search: ctx.movie ? (ctx.movie.title||ctx.movie.name) : ''
      });
    });

    // стараемся встать рядом с "Онлайн" (view--torrent); если нет — просто в блок кнопок
    var inserted = false;
    try{
      var container = ctx.render.closest('.full-start__buttons');
      if (container && container.length){
        ctx.render.after(btn);
        inserted = true;
      }
    }catch(e){/* no-op */}
    if (!inserted){
      try{
        var fallback = $('.full-start__buttons').first();
        if (fallback.length) fallback.append(btn);
      }catch(e){}
    }
  }
  ...
  function mount(){
    ...
    // после построения полной карточки
    Lampa.Listener.follow('full', function(ev){
      if (ev.type === 'complite'){
        var render = ev.object && ev.object.activity ? ev.object.activity.render().find('.view--torrent') : $();
        addButton({ render: render, movie: ev.data.movie });
      }
    });

    // если уже на карточке
    try{
      if (Lampa.Activity.active().component === 'full'){
        var r = Lampa.Activity.active().activity.render().find('.view--torrent');
        addButton({ render: r, movie: Lampa.Activity.active().card });
      }
    }catch(e){}
  }

  // простой CSS-фолбэк на случай отсутствия онлайн-стилей
  if (!$('.online-prestige').length){
    $('body').append(
      "<style>.online-prestige{position:relative;border-radius:.3em;background:rgba(0,0,0,.3);display:flex}.online-prestige__body{padding:1.2em;flex-grow:1;position:relative}.online-prestige__info{display:flex;align-items:center}.online-prestige-split{font-size:.8em;margin:0 1em;flex-shrink:0}</style>"
    );
  }
